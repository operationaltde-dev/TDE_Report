...
use App\Exports\DoorAlarmReport as DoorAlarmReportExport;
use Carbon\Carbon;

use App\Models\EventLog as EventLogModel;
use App\Models\PartitionMaster;

class DoorAlarmReport extends Component
{
    use WithPagination;
    ...
    private function baseQuery()
    {
        return EventLogModel::with('credential.owner.group')
            ->where(function ($q) {
                $q->where('msgdescription', 'ilike', '%normal%')
                  ->orWhere('msgdescription', 'ilike', '%alarm%');
            })
            ->where('msgdescription', 'not ilike', '%sensor%')
    ...
    public function exportPdf()
    {
        ...
        $perRow = 20;
        $query = $this->baseQuery()->orderBy('ts', 'asc');
        ...
        if ($totalRows < $chunkSize) {
            ...
            $html = view('pdf.door-alarm-report', [
                'reports'         => $reports,
            ...
            $pdfPath = storage_path(
                "app/Door-Alarm-Report_{$start}_to_{$end}.pdf"
            );
        ...
        $query->chunk($chunkSize, function ($reports) use (
            ...
            $path = storage_path(
                "app/temp/Door-Alarm-Report_{$start}_to_{$end}_Part-{$index}.pdf"
            );
        ...
        $zipPath = storage_path(
            "app/Door-Alarm-Report_{$start}_to_{$end}.zip"
        );
    ...
    public function exportExcel()
    {
        ...
        return Excel::download(
            new DoorAlarmReportExport($this->baseQuery()),
            "Door-Alarm-Report_{$start}_to_{$end}.xlsx"
        );
    }

    public function render()
    {
        return view('livewire.dashboard.transaction.door-alarm-report', [
            'door_alarm_reports' => $this->baseQuery()->orderBy('ts', 'desc')->paginate($this->perPage),
            'locations' => PartitionMaster::where('isdeleted', false)->get()
        ])
        ->layout('components.layouts.dashboard')
        ->title('Door Alarm Report');
    }
}